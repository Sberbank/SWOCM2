 ...
 * is communicated by injecting a 1x1 pixel image into the page. The base URL of
 * the tracking image is defined in the function "ibm_analytics_SampleAggregator_init"
 * at the bottom. Before loading the tracking image, the aggregator will append the
 * collected data as a query sting to the image URL. The resulting URL will look like
 * this: http://example.org/tracking.gif?asa.page.title=My%20Page&asa.visitor=user4...
 *
 ...
 */
if (typeof(ibm_analytics_SampleAggregator) === "undefined") {

/**
 * Function to construct the aggregator
 */
ibm_analytics_SampleAggregator = function (/*String*/trackingImageURL) {
...
// callback function to register the aggregator
var ibm_analytics_SampleAggregator_init = function() {
  // instantiate the aggregator
  var imgURL = "http://example.org/tracking.gif";
  var interval = 30000;
  ibm_sample_aggregator = new ibm_analytics_SampleAggregator(imgURL);
  // activate the send interval
  setInterval(ibm_sample_aggregator.submit, interval);
  // register the aggregator
  com.ibm.portal.analytics.SiteAnalyticsMediator.register(function() {
    ibm_sample_aggregator.parse.apply(ibm_sample_aggregator, arguments);
  });
};

// callback function to submit the collected data to the external analytics service
var ibm_analytics_SampleAggregator_submit = function() {
  ibm_sample_aggregator.submit.apply(ibm_sample_aggregator, arguments);
};

// register the callbacks for registration and data submission
if (window.addEventListener) {
  // W3C
  window.addEventListener("load", ibm_analytics_SampleAggregator_init, false);
  window.addEventListener("beforeunload", ibm_analytics_SampleAggregator_submit, false);
} else if (window.attachEvent) {
  // Microsoft
  window.attachEvent("onload", ibm_analytics_SampleAggregator_init);
  window.attachEvent("onunload", ibm_analytics_SampleAggregator_submit);
}