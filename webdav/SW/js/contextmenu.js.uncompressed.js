		...
		contextMenu: {
			...
			init: function(node, menuId, jsonQuery){
				...
				var callback = function(){
					// show menu
					i$.addClass(i$.byId(jsonData.id), wptheme.contextMenu.css.show);
					// set focus here
					...
					i$.bindDomEvt(i$.byId(jsonData.id), "onmouseleave", function(){
						var _node = i$.byId(jsonData.id);
						i$.removeClass(_node, wptheme.contextMenu.css.show);
			...
			_initialize: function(node){
				...
				if (jsonData._submenu) {
					...
					// find nodes first
					var templateNodes = wptheme.contextMenu._findNodes(node);
				...
				i$.addClass(node, wptheme.contextMenu.css.show);
				...
				return wptheme.contextMenu._load(jsonData).then(function(data){
					var retVal = wptheme.contextMenu._parseData(data).then(function(items){
						...
						if (!items || items.length == 0) {
							...
							tmp.innerHTML = wptheme.contextMenu._fromTemplate(jsonData._menuitemTemplate, wptheme.contextMenu.css.error, "No items to display.");
							while (menuNode.firstChild) {
								menuNode.removeChild(menuNode.firstChild);
							}
					...
					var tmp = i$.createDom("div");
					tmp.innerHTML = wptheme.contextMenu._fromTemplate(jsonData._menuitemTemplate, wptheme.contextMenu.css.error, "Error happened while loading the menu.");
					while (menuNode.firstChild) {
						menuNode.removeChild(menuNode.firstChild);
					}
					menuNode.appendChild(tmp);
						...
						if (childnode.nodeType == 1) {
							...
							if (menuitem.type == "Submenu") {
								...
								i$.bindDomEvt(childnode, "onclick", function(evt){
									...
									setTimeout(function(){
										i$.removeClass(i$.byId(jsonData.id), wptheme.contextMenu.css.show);
									}, 0);
								});
			...
			_applyKeyAction: function(evt){
				...
				switch (evt.keyCode) {
					case 13: { // ENTER
						...
						i$.removeClass(i$.byId(jsonData.id), wptheme.contextMenu.css.show);
						// move focus
						var focusNode = wptheme.contextMenu._findFocusNode(menuNode);
